...
const store = new Vuex.Store({
    ...
    contrastFaceVerify(commit, payload) {
      return new Promise((resolve, reject) => {
        request('post', {
          service: 'voucher.face.contrastFaceVerify',
          userId: commit.state.userId,
          ...payload
        }).then(response => {
          resolve(response);
	...
	mutations: {
      ...
      state.certificated = payload.certificated;
			localStorage.setItem("userId", payload.userId)
		},
    changeCertificated(state, payload) {
      state.certificated = payload;
    }
	}
})
